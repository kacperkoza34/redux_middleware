...
const jsonServerUrl = `${window.location}books`;
...
const getBooks = ({dispatch, getBooks}) => next => action => {
  ...
  if ( action.type === DELETE_BOOK  ) {
    dispatch(apiRequest('DELETE', `${jsonServerUrl}/${action.payload}`, deleteOne, error, action.payload));
  }

  if ( action.type === ADD_BOOK  ) {
    dispatch(apiRequest('POST', jsonServerUrl, addOne, error, {title: action.payload}));
  }
